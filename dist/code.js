...
let parentNodes = []; // Store the parent nodes globally
// Show UI
    ...
    if (msg.type === 'get-values') {
        ...
        for (const rootNode of selection) {
            ...
            if ('findAll' in rootNode) {
                const foundNodes = rootNode.findAll((node) => node.name === parentLayerName);
...
//EXTERNAL FUNCTIONS
function updateAllTextLayers(parentNodes, values) {
...
function logNodeDetails(node) {
    ...
    console.log(`Found node: ${node.name || "[Unnamed Node]"} | Type: ${node.type} | Text: "${nodeText}"`);
}
...
function rgbToHct(r, g, b) {
    try {
        ...
        // Create HCT color
        const hct = material_color_utilities_1.Hct.fromInt((red << 16) + (green << 8) + blue);
